...
class DrivableMap {
    ...
    public boolean hasFasterThan(int speed){
        int max = 0;
        Collection<Drivable> col = drivable_map.values();
        for (Drivable item: col){
    ...
    public ArrayList<Tradable> getTradable(){
        ArrayList<Tradable> arr = new ArrayList<>();
        Collection<String> col = drivable_map.keySet();
        for (String str: col){
            if(drivable_map.get(str) instanceof Tradable){
                arr.add((Tradable) drivable_map.get(str));
            }
        }
        return arr;
    }
